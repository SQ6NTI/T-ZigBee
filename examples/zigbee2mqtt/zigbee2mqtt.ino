...
void ledTask(void *pvParameters) {
    while (1) {
        if (staStatus == "n/a") {
            ...
            digitalWrite(CONFIG_BLUE_LIGHT_PIN, HIGH);
            delay(100);
            if (topicsSubscribed != "yes") {
                subMQTTAttrGet();
                topicsSubscribed = "yes";
            }
        }
    }
    vTaskDelete(NULL);
...
void appWiFiInit(void) {
    ...
    //Serial.println("WiFi: Set mode to WIFI_AP");
    // WiFi.mode(WIFI_AP);
...
void appHandleLeaveIndication(ts_MsgLeaveIndicationPayload *payload) {
    ...
    if (!payload->u8Rejoin) {
        ...
        else
        {
            ...
            {
                tuyaPlugDelete(device->u64IeeeAddr);
            }
        }
...
void appHandleZCLreportMsgRcv(ts_MsgZclReportMsgRcvPayload *payload) {
    ...
    switch (payload->u16ClusterId) {
            ...
            for (size_t i = 0; i < payload->u8AttrNum; i++)
            {
                if (payload->asAttrList[i].u16AttrID == 0x0000)
                {
                        ...
                        !strncmp((const char *)device->au8ModelId, "ESP32C6.Light", strlen("ESP32C6.Light")))
                    {
                        ...
                        if (device->u64IeeeAddr == 0xa4c1387ca2489e61)
                        {
                            device->deviceData.ts011f.bOnOff = payload->asAttrList[i].uAttrData.u8AttrData;
                            tuyaPlugReport(device->u64IeeeAddr, device->deviceData.ts011f.bOnOff);
                        }
                    }
                }
            }
        ...
        case 0xe000: /* Tuya Common Private Cluster */
        {
            ...
            {
                appDataBaseSave();
                tuyaPlugAdd(device->u64IeeeAddr);
            }
        }
